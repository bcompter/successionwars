...
class Ion_auth
{
	...
	{
		log_message('error', 'AUTH: '.$code);
		
		$profile = $this->ci->db->query('SELECT * FROM users WHERE forgotten_password_code="'.$code.'"')->row();
		log_message('error', 'Debug query id is '.$profile->id);
		...
		//log_message('error', 'auth controller profile is '.$profile);

		if (!is_object($profile))
		//if ($profile != 1)
		{
			log_message('error', 'AUTH not is object.');
			$this->set_error('password_change_unsuccessful');
			return FALSE;
		}
		//$profile = $this->ci->db->query('SELECT * FROM users WHERE forgotten_password_code="'.$identity.'"')->row();

		$new_password = $this->ci->ion_auth_model->forgotten_password_complete($code, $profile->salt);

		log_message('error', 'New password is '.$new_password);
		...
		if ($new_password)
		{
			...
			{
				log_message('error', 'AUTH else');
				$this->set_error('password_change_unsuccessful');
				return FALSE;
			}
		}
		
		log_message('error', 'AUTH end of logic.');
		$this->set_error('password_change_unsuccessful');
		return FALSE;
	}
